...
export const ComponentPreview: React.FC<ComponentPreviewProps> = ({ 
  ...
  const [scrollInPreview, setScrollInPreview] = useState(true);
  ...
  return (
    <div className={`relative w-full ${t.mainBg} ${t.mainText} font-inter flex items-center justify-center py-4 md:py-8 transition-colors duration-500 rounded-2xl mb-12`}>
      ...
      <div className="relative z-10 w-full max-w-6xl flex flex-col gap-6 px-4">
        ...
        {/* Playground Box */}
        <div className={`${t.editorCardBg} backdrop-blur-2xl border ${t.cardBorder} rounded-2xl overflow-hidden shadow-2xl flex flex-col ${scrollInPreview ? 'h-[700px]' : 'min-h-[700px] h-auto'} ring-1 ${t.cardRing} transition-all duration-500`}>
              ...
              {activeTab === 'preview' && (
                <>
                  <div className={`hidden sm:flex items-center p-1 ${t.segmentBg} rounded-lg border ${t.segmentBorder} transition-colors duration-500`}>
                    <button
                      onClick={() => setScrollInPreview(!scrollInPreview)}
                      className={`px-3 py-1.5 rounded-md text-sm font-medium transition-all duration-200 ${scrollInPreview ? t.deviceActiveBg : t.deviceInactiveText}`}
                      title="Toggle Scrollable Preview"
                    >
                      {scrollInPreview ? 'Fixed Height (Scrollable)' : 'Full Height'}
                    </button>
                  </div>
                  <div className={`hidden sm:flex items-center p-1 ${t.segmentBg} rounded-lg border ${t.segmentBorder} transition-colors duration-500`}>
                    <button
          ...
          {/* Body */}
          <div className={`flex-1 relative flex flex-col ${t.playBg} ${scrollInPreview ? 'overflow-hidden' : 'overflow-visible'} transition-colors duration-500`}>
            
            {/* Preview Section */}
            <div className={`${activeTab === 'preview' ? (scrollInPreview ? 'absolute inset-0' : 'relative flex-1') : 'absolute inset-0'} flex items-center justify-center ${scrollInPreview ? 'overflow-hidden' : 'overflow-visible'} p-4 custom-scrollbar transition-opacity duration-300 ${activeTab === 'preview' ? 'opacity-100 z-10' : 'opacity-0 pointer-events-none z-0'}`}>
              <div 
                className={`${scrollInPreview ? 'h-full max-h-[800px]' : 'h-auto w-full'} transition-all duration-500 ease-in-out origin-center ${getPreviewWidth()}`}
              >
                <div className={`${scrollInPreview ? 'h-full' : 'h-auto'} w-full ${t.deviceBezel} ${device !== 'desktop' ? 'rounded-[2.5rem] border-[12px] shadow-2xl' : 'rounded-b-lg'} ${scrollInPreview ? 'overflow-y-auto overflow-x-hidden' : 'overflow-visible'} custom-scrollbar relative transition-colors duration-500`}>
            ...
            {/* Code Section */}
            <div className={`${activeTab === 'code' ? (scrollInPreview ? 'absolute inset-0' : 'relative flex-1 min-h-[600px]') : 'absolute inset-0'} flex flex-col transition-opacity duration-300 ${t.codeWrapBg} ${activeTab === 'code' ? 'opacity-100 z-10' : 'opacity-0 pointer-events-none z-0'}`}>
